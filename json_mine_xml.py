...
#"1605.02019"
#"9610131"
directory = "loading_dock/json_latexml/"
...
def format_xml_math_space():
	...
	with open(directory + filename ) as fn:
		for line in fn:
			...
			if(re.search(",", ME.ref_recovery_location[ref_loc])):
				...
				if(ME.ref_recovery_location[ref_loc] in ME.me_ref_list):
					if(len(data) == 1):
						...
						try:
							data = data[0] + ME.me_ref_list[ME.ref_recovery_location[ref_loc]]
						except:
							data = data[0] + "[REF:ERROR]"
					else:
						try:
							data = data[0] + ME.me_ref_list[ME.ref_recovery_location[ref_loc]] + data[1]
						except:
							data = data[0] + "[REF:ERROR]" + data[1]
				ME.xml_lines[ref_loc] = data
...
def outfile_data():
	#Write to out file
	print("HERE_OUTFILE")
	directory = filename + "_jsondata/"
	if not os.path.exists(directory):
		os.makedirs(directory)
	out_file = open(filename + "_jsondata/" + filename + "_xmlmined.txt", "w")
	out_file.write(ME.xml_text)
	out_file.close()

	out_file = open(filename + "_jsondata/" + filename + "_melist.txt", "w")
	for me in ME.me_latex:
		me = me + "\n"
		out_file.write(me)
	out_file.close()	
	out_file = open(filename + "_jsondata/" + filename + "_merefnumlist.txt", "w")
	...
	with open(directory + filename + "_formated_final.xml") as fn:
		for line in fn:
			...
			try:
				...
				print("RE SEARCH FAIL LINE 409-451")
			...
			if(reference_flag == 0):
				...
				try:
					...
					print("UNKOWN BULLSHIT at 516")
				#print words

				#print("\n\n")
				if(len(words) == 1):

					ME.xml_lines.append(words[0])
					ME.xml_text = ME.xml_text + words[0]
				else:
